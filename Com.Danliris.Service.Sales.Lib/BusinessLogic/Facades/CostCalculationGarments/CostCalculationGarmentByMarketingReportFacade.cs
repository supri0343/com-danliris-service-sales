...
{
    public class CostCalculationGarmentByMarketingReportFacade : ICostCalculationGarmentByMarketingReport
    {
        private readonly SalesDbContext DbContext;
        private readonly DbSet<CostCalculationGarment> DbSet;
        private IdentityService IdentityService;
        private CostCalculationByMarketingReportLogic CostCalculationByMarketingReportLogic;

        public CostCalculationGarmentByMarketingReportFacade(IServiceProvider serviceProvider, SalesDbContext dbContext)
        {
            this.DbContext = dbContext;
            this.DbSet = this.DbContext.Set<CostCalculationGarment>();
            this.IdentityService = serviceProvider.GetService<IdentityService>();
            this.CostCalculationByMarketingReportLogic = serviceProvider.GetService<CostCalculationByMarketingReportLogic>();
        }
        
        public Tuple<MemoryStream, string> GenerateExcel(string filter = "{}")
        {
            var Query = CostCalculationByMarketingReportLogic.GetQuery(filter);
            ...
            result.Columns.Add(new DataColumn() { ColumnName = "Marketing Garment", DataType = typeof(String) });
            ...
            {
                    Dictionary<string, List<CostCalculationGarmentByMarketingReportViewModel>> dataByBrand = new Dictionary<string, List<CostCalculationGarmentByMarketingReportViewModel>>();
                ...
                foreach (CostCalculationGarmentByMarketingReportViewModel item in Query.ToList())
                {
                    string BrandName = item.BrandName;
                    if (!dataByBrand.ContainsKey(BrandName)) dataByBrand.Add(BrandName, new List<CostCalculationGarmentByMarketingReportViewModel> { });
                        dataByBrand[BrandName].Add(new CostCalculationGarmentByMarketingReportViewModel
                        {
                    ...
                    foreach (KeyValuePair<string, List<CostCalculationGarmentByMarketingReportViewModel>> BuyerBrand in dataByBrand)
                    {
                        string BrandCode = "";
                        int index = 0;
                        foreach (CostCalculationGarmentByMarketingReportViewModel item in BuyerBrand.Value)
                        {
                            index++;
                            ...
                            result.Rows.Add(index, item.RO_Number, CfrmDate, ShipDate, item.UnitName, item.Section, item.MarketingName, item.Article, item.Comodity, item.Description,
            ...
            var sheet = package.Workbook.Worksheets.Add("COST CALCULATION PER MARKETING");
            ...
            string fileName = string.Concat("Cost Calculation Per Marketing", ".xlsx");

            return Tuple.Create(streamExcel, fileName);
        }

        public Tuple<List<CostCalculationGarmentByMarketingReportViewModel>, int> Read(int page = 1, int size = 25, string filter = "{}")
        {
            var Query = CostCalculationByMarketingReportLogic.GetQuery(filter);
            var data = Query.ToList();
